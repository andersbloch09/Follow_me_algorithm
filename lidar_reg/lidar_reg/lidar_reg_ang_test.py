...
def reg_ang(target_x, target_y, orientation, robotx, roboty, angvel): 
    ...
    #response = (err,angvel)
    #with open("ResponseGraphANGERR.txt",'a') as f:
    #    line= str(response)
    #    f.write(line+'\n')
...
class linear_Sub(Node):
    ...
    def listener_callback(self,msg: JointState):
        self.vel = (msg.velocity[0]+msg.velocity[1])/2
...
def main():
    ...
    queue.append([2.5,1])
    queue.append([3,0])
    queue.append([3.5,-0.5])
    queue.append([4,0.0])
    queue.append([4.5,0.5])
    queue.append([5,0.0])
    queue.append([5.5,-0.5])
    ...
    while True: 
        ...
        while distToWP > goalDist:
            ...
            response = (time.time(),Imu_node.angvel)
            with open("ResponseGraphLINVELTIMENEW.txt",'a') as f:
                 line= str(response)
                 f.write(line+'\n')

            
            # If more than x waypoints exist, remove the first one in the queue until its below the limit
            ...
            #response = (control_angular_velocity,Imu_node.angvel)
            #with open("ResponseGraphANGVELCONTROL.txt",'a') as f:
            #     line= str(response)
            #     f.write(line+'\n')
