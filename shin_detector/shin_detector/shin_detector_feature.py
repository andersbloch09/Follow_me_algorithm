...
def getFeatures(clust_x, clust_y, clustersize):
            ...
            features=[clustersize, std, avg_med_dev, linearity, circularity,
                                radius]
            with open("negative_Features.txt",'a') as f:
                 line= str(features)
                 f.write(line+'\n')
...
def main(args=None):
    ...
    global vectorArrX
    global vectorArrY
    vectorArrX = np.zeros(50)
    vectorArrY = np.zeros(50)
    ...
    while(1):
        ...
        db = DBSCAN(eps=0.05, min_samples=3,).fit(coord)
        ...
        for i in range(n_clusters_):
            ...
            getFeatures(tempX,tempY,len(tempX))
